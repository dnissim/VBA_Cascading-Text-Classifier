...
'================================DEBUGGING/TUNING=======================================
    'Function TestGuessValue(strInput As String) As String
        ''For testing transaction code guessing
        'Dim arrSearch(), arrResult()
        '
        'arrSearch = RangeToArray(TableColumnRange(GetTable("tblTrans"), "Location"))
        'arrResult = RangeToArray(TableColumnRange(GetTable("tblTrans"), "Code"))
        '
        '
        'Dim a As Guess
        'TestGuessValue = GuessValue(strInput, arrSearch, arrResult).strGuess
        'End Function
        '
        'Private Sub outputGuessResults(Col As Collection, _
        '                               arr As Variant, _
        ...
        '
        '.Cells(1, 1) = "Result"
        '.Cells(1, 2) = "Match Count"
        '.Cells(1, 4) = "Search Value"
        '.Cells(1, 5) = SearchValue
        '.Cells(2, 4) = "Guess Value"
        '.Cells(2, 5) = GuessValue.strGuess
        '.Cells(3, 4) = "Matches"
        '.Cells(3, 5) = GuessValue.Matches
        '.Cells(4, 4) = "Proportion"
        '.Cells(4, 5) = GuessValue.Proportion
        '
        'For i = 1 To Col.count
        '    .Cells(i + 1, 1) = Col(i)
        'Next i
        '
        '.Range(.Cells(2, 2), .Cells(Col.count + 1, 2)).Value = Application.WorksheetFunction.Transpose(arr)
        '
        '.Activate
        '
        'End With
    ...
    '#####
    'Private Sub DetermineOptimalParameters()
        '
        'Dim minTolerance As Single, deltaTolerance As Single
        'Dim minMatches As Integer
        ...
        '
        'Set inRange = shtTest.Range("A2:B1500")
        'Set outRange = shtTest.Range("E2:F1500")
        '
        'arrHolder = RangeToArray(inRange)
        '
        'Set inRange = Nothing
        'Set tblResults = GetTable("tblGuessOptimization")
        '
        ...
        '    deltaTolerance = 0.02
        '    minMatches = 1
        '    RowResult = shtTest.Cells(shtTest.Rows.count, "k").End(xlUp).Row + 1
        '    '=================
        '
        '    Do While minTolerance < 1
        '        For minMatches = 1 To 2
        '            RowResult = RowResult + 1
        '            startTime = Timer
        '            For rowData = LBound(arrHolder) To UBound(arrHolder)
        '               curTrans.Location = arrHolder(rowData, 2)
        '               curTrans.Source = arrHolder(rowData, 1)
        '               arrHolder(rowData, 1) = GuessCategory(curTrans, minMatches, minTolerance)
        '            Next rowData
        '            endtime = Timer
        '
        '            outRange.Value2 = arrHolder
        '
        '            .Calculate
        '
        '            .Cells(RowResult + 1, 10) = minMatches
        '            .Cells(RowResult + 1, 11) = minTolerance
        '            .Cells(RowResult + 1, 12) = .Range("H2")
        '            .Cells(RowResult + 1, 13) = .Range("H3")
        '            .Cells(RowResult + 1, 14) = .Range("H4")
        '            .Cells(RowResult + 1, 15) = endtime - startTime
        '
        '            'Debug.Print "MinMatches: " & minMatches & ", minTolerance: " & minTolerance
        '
        '        Next minMatches
        '        minTolerance = minTolerance + deltaTolerance
        '    Loop
        'End With
